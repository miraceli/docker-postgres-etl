...
def wait_for_postgres(host, port, max_retries=5, delay_seconds=5):
    """Wait for PostgreSQL to become available."""
    retries = 0
    while retries < max_retries:
        try:
            result = subprocess.run(
                ["pg_isready", "-h", host, "-p", port], check=True, capture_output=True, text=True)
...
if not wait_for_postgres(host="source_postgres", port="5432"):
    exit(1)
